...
var Alarm_ID1 = false;			// Trigger Alarm add ID1_LỖI SETPOINT
var Alarm_ID2 = false;			// Trigger Alarm add ID2_LỖI MẤT TH LEVEL
var Alarm_ID3 = false;			// Trigger Alarm add ID3_LỖI MẤT TH FLOW
var Alarm_ID4 = false;			// Trigger Alarm add ID4_LỖI MẤT TH PRESSURE
...
app.get("/control", function (req, res) {
  res.render("layout", { screenurl: "./screen/control.ejs" });
});
// system calling
...
app.get("/trend", function (req, res) {
  res.render("layout", { screenurl: "./screen/trend.ejs" });
});
// DATA calling
app.get("/data", function (req, res) {
  res.render("layout", { screenurl: "./screen/data.ejs" });
});
// alarm calling
...
conn_plc.initiateConnection({ port: 102, host: "192.168.0.11", rack: 0, slot: 1 }, PLC_connected);
...
var tags_list = {
  ...
  btt_SP: "DB6,X4.7",
  SP_Mode: "DB6,X5.0",
  btt_loi_level: "DB6,X5.1",
  btt_loi_flow: "DB6,X5.2",
  btt_loi_pressure: "DB6,X5.3",
  WARNING_LEVEL: "DB6,X5.4",
  WARNING_FLOW: "DB6,X5.5",
  WARNING_PRESSURE: "DB6,X5.6",
  //tag dữ liệu Real
  SP: "DB6,REAL6",
  PV: "DB6,REAL10",
  Level: "DB6,REAL14",
...
function fn_tag() {
  ...
  io.sockets.emit("btt_SP", arr_tag_value[35]);
  io.sockets.emit("SP_Mode", arr_tag_value[36]);
  io.sockets.emit("btt_loi_level", arr_tag_value[37]);
  io.sockets.emit("btt_loi_flow", arr_tag_value[38]);
  io.sockets.emit("btt_loi_pressure", arr_tag_value[39]);
  io.sockets.emit("WARNING_LEVEL", arr_tag_value[40]);
  io.sockets.emit("WARNING_FLOW", arr_tag_value[41]);
  io.sockets.emit("WARNING_PRESSURE", arr_tag_value[42]);
  //BIT_REAL//
  io.sockets.emit("SP", arr_tag_value[43]);
  io.sockets.emit("PV", arr_tag_value[44]);
  io.sockets.emit("Level", arr_tag_value[45]);
  io.sockets.emit("KP", arr_tag_value[46]);
  io.sockets.emit("KI", arr_tag_value[47]);
  io.sockets.emit("KD", arr_tag_value[48]);
  io.sockets.emit("Flow", arr_tag_value[49]);
  io.sockets.emit("Pressure", arr_tag_value[50]);
  io.sockets.emit("SP_Real", arr_tag_value[51]);
}
...
io.on("connection", function (socket) {
  ...
  // NÚT sp_mode
  socket.on("cmd_SP_MODE", function (data) {
    conn_plc.writeItems("btt_SP", data, valuesWritten);
  });
  // NÚT ON BOM
  ...
  // NÚT LỖI 1_LEVEL
  socket.on("cmd_default1", function (data) {
    conn_plc.writeItems("btt_loi_level", data, valuesWritten);
  });
  // NÚT LỖI 2_FLOW
  socket.on("cmd_default2", function (data) {
    conn_plc.writeItems("btt_loi_flow", data, valuesWritten);
  });
  // NÚT LỖI 3_PRESSURE
  socket.on("cmd_default3", function (data) {
    conn_plc.writeItems("btt_loi_pressure", data, valuesWritten);
  });
  // Ghi dữ liệu từ IO field
  socket.on("cmd_Edit_Data", function (data) {
    conn_plc.writeItems(["SP_Real"], [data[0]], valuesWritten);
  });
});
...
///////////// CHƯƠNG TRÌNH CON THÊM DỮ LIỆU VÀO BẢNG SQL///////////////////////
function fn_sql_insert() {
  ...
  var data_SP = "'" + arr_tag_value[43] + "',";
  var data_Level = "'" + arr_tag_value[45] + "',";
  var data_Flow = "'" + arr_tag_value[49] + "',";
  var data_Pressure = "'" + arr_tag_value[50] + "'";
...
function fn_Alarm_Manage(){
  Alarm_ID1 = arr_tag_value[6];		// Read trigger alarm ID1
  Alarm_ID2 = arr_tag_value[40];		// Read trigger alarm ID2
  Alarm_ID3 = arr_tag_value[41];		// Read trigger alarm ID3
  Alarm_ID4 = arr_tag_value[42];		// Read trigger alarm ID4
  // Cảnh báo lỗi giới hạn SP
  if (Alarm_ID1 && !Alarm_ID1_old){
      fn_sql_alarm_insert(1, "SETPOINT VƯỢT QUÁ GIỚI HẠN CHO PHÉP ")
  } if(Alarm_ID1 == false & Alarm_ID1 != Alarm_ID1_old) {
      fn_sql_alarm_ack(1);
  }
  Alarm_ID1_old = Alarm_ID1;
  // Cảnh báo lỗi mất tín hiệu level
  if (Alarm_ID2 && !Alarm_ID2_old){
      fn_sql_alarm_insert(2, "MẤT TÍN HIỆU LEVEL")
  } if(Alarm_ID2 == false & Alarm_ID2 != Alarm_ID2_old) {
      fn_sql_alarm_ack(2);
  }
  Alarm_ID2_old = Alarm_ID2;
  // Cảnh báo lỗi mất tín hiệu flow
  if (Alarm_ID3 && !Alarm_ID3_old){
      fn_sql_alarm_insert(3, "MẤT TÍN HIỆU FLOW")
  } if(Alarm_ID3 == false & Alarm_ID3 != Alarm_ID3_old) {
      fn_sql_alarm_ack(3);
  }
  Alarm_ID3_old = Alarm_ID3; 
  // Cảnh báo lỗi mất tín hiệu pressure
  if (Alarm_ID4 && !Alarm_ID4_old){
    fn_sql_alarm_insert(4, "MẤT TÍN HIỆU PRESSURE")
...
function fn_Alarm_Show(){
  io.on("connection", function(socket){
      ...
      {
          var sqltable_Name = "alarm";
          var query_1 = "SELECT * FROM " + sqltable_Name + " WHERE Status = 'ERROR';"; 
         
          
          // var query_2 = "SELECT * FROM " + sqltable_Name1 + " WHERE Status = 'NO ERROR';";
          // var query   = query_1 + query_2 + ";";
          sqlcon.query(query_1, function(err, results, fields) {
              if (err) {
                  console.log(err);
              } else {
                  const objectifyRawPacket = row => ({...row});
